  ...
  exclude-result-prefixes="marc items">
   ...
   <xsl:if test="$DisplayOPACiconsXSLT!='0'">
        ...
        <xsl:if test="marc:controlfield[@tag=001]">
            <span class="results_summary"><span class="label">Accession Number: </span>
                <xsl:variable name="accession_number" select="marc:controlfield[@tag=001]"/>
...
-->
       ...
       <xsl:if test="marc:datafield[@tag=020]">
        ...
        <xsl:if test="marc:datafield[@tag=830]">
        <span class="results_summary uniform_series_title"><span class="label">In: </span>
        <xsl:for-each select="marc:datafield[@tag=830]">
            <xsl:variable name="str">
                <xsl:for-each select="marc:subfield">
                    <xsl:if test="(contains('adfklmorv',@code) and (not(../marc:subfield[@code='n' or @code='p']) or (following-sibling::marc:subfield[@code='n' or @code='p'])))">
                        <xsl:value-of select="text()"/>
                        <xsl:text> </xsl:text>
                     </xsl:if>
                </xsl:for-each>
        ...
        <xsl:if test="marc:datafield[@tag=650] or marc:datafield[@tag=651]">
...
<!-- Image processing code added here, takes precedence over text links including y3z text   -->
        ...
        <span class="results_summary online_resources"><span class="label">Online Availability: </span>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
            ...
            <xsl:when test="marc:subfield[@code='y' or @code='z']">
                <xsl:call-template name="subfieldSelect">
                    <xsl:with-param name="codes">yz</xsl:with-param>
                </xsl:call-template>
            </xsl:when>
        ...
        <!--  775 Other Edition/Availability  -->
        <xsl:if test="marc:datafield[@tag=775]">
        <span class="results_summary other_editions"><span class="label">Other language: </span>
        <xsl:for-each select="marc:datafield[@tag=775]">
            ...
            <a>
            <xsl:choose>
            <xsl:when test="$UseControlNumber = '1' and marc:subfield[@code='o']">
                <xsl:attribute name="href">/cgi-bin/koha/opac-search.pl?q=kw:<xsl:value-of select="marc:subfield[@code='o']"/></xsl:attribute>
            </xsl:when>
            <xsl:when test="$UseControlNumber = '1' and marc:subfield[@code='w']">
                <xsl:attribute name="href">/cgi-bin/koha/opac-search.pl?q=Control-number:<xsl:call-template name="extractControlNumber"><xsl:with-param name="subfieldW" select="marc:subfield[@code='w']"/></xsl:call-template></xsl:attribute>
            </xsl:when>
        ...
        <!-- 776 Other Form/Availability -->
        <xsl:if test="marc:datafield[@tag=776]">
        <span class="results_summary"><span class="label">Alternate format: </span>
...
<xsl:if test="marc:datafield[@tag=041]">
    <span class="results_summary">
        ...
        <xsl:when test="@ind2=2">
            <span class="label">Successive title:</span>
        </xsl:when>
        <xsl:when test="@ind2=3">
...
<xsl:if test="marc:datafield[@tag=500]">
    <span class="results_summary"><span class="label">General notes:
        </span>
